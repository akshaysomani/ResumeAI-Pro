...
import { getOrgBillingAction } from "@/app/actions/orgActions";
...
export default async function BillingCenterPage({ params }: PageProps) {
  ...
  // 4. Fetch billing rows
  const billingInfo = await getOrgBillingAction(orgId);

  // 5. Fetch members count
  const membersRes = await db.query(
    `SELECT COUNT(user_id)::int as count FROM public.organization_members WHERE organization_id = $1`,
    [orgId]
  );
  const activeMembersCount = membersRes.rows[0]?.count || 0;
